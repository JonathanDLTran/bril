...
export class RefMap {
  ...
  add_ptr(parent: Pointer, child: Pointer) {
    if (this.pointermap.has(parent)) {
      this.pointermap.get(parent)!.push(child);
    } else {
      this.pointermap.set(parent, [child]);
    }
  }

  remove_ptr(parent: Pointer) {
    this.pointermap.delete(parent);
  }

  add_key(ptr: Pointer) {
    this.refmap.set(ptr, 0);
  }

  remove_key(ptr: Pointer) {
    this.refmap.delete(ptr);
  }

  get_all_ptrs(): Pointer[] {
    let pointers = [];
    for (let [ptr, _] of this.refmap) {
      pointers.push(ptr);
    }
    return pointers;
  }

  ref_count(ptr: Pointer): number {
    let count = this.refmap.get(ptr);
    if (count != undefined) {
      return count;
    }
    throw error(`Count ${count} was not a number for pointer ${ptr}.`)
  }

  incr(ptr: Pointer, n: number): number {
    let count = this.refmap.get(ptr);
    if (count !== undefined && count >= 0) {
      let new_count = count + n;
      this.refmap.set(ptr, new_count);
      return new_count
    } else {
      throw error(`Pointer ${ptr} does not exist or count ${count} was negative`);
    }
  }

  decr(ptr: Pointer, n: number): number {
    let count = this.refmap.get(ptr);
  ...
  recursively_decr(ptr: Pointer, heap: Heap<Value>, visited: Pointer[]) {
    this.decr(ptr, 1);
    ...
    if (this.ref_count(ptr) === 0) {
      ...
      if (this.pointermap.has(ptr)) {
        let children = this.pointermap.get(ptr);
        for (let child of children!) {
          if (!visited.includes(child)) {
            this.recursively_decr(child, heap, visited);
          }
        }
      }
...
function isPtr(instr: bril.Operation, env: Env, index: number): boolean {
...
function getArgumentName(instr: bril.Operation, index: number) {
  let args = instr.args || [];
...
type State = {
  ...
  // For garbage collection, an array of refmaps for each function is created
  refmap: RefMap,
}
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  switch (instr.op) {
    ...
    case "id": {
      ...
      // check if the argument is a pointer, which implies dest is a pointer
      if (isPtr(instr, state.env, 0)) {
        let argptr = state.env.get(getArgumentName(instr, 0));
        let destptr = state.env.get(instr.dest);
        if (destptr === undefined) {
          throw error(`${destptr} cannot be a undefined`);
        }
        if (argptr !== undefined) {
    ...
    case "alloc": {
      ...
      state.refmap.add_key(ptr);
      state.refmap.incr(ptr, 1);
      return NEXT;
    }
    ...
    case "ptradd": {
      ...
      // increment pointer if instr.dest is not the same as first arg
      // if the dest and arg names are identical, do nothign
      if (instr.dest != getArgumentName(instr, 0)) {
        state.refmap.incr(ptr, 1);
      }
...
function evalProg(prog: bril.Program) {
  ...
  let unfreed_pointers = state.refmap.get_all_ptrs();
  for (let ptr of unfreed_pointers) {
    let count = state.refmap.ref_count(ptr);
    if (count !== 0) {
      state.heap.free(ptr.loc);
      state.refmap.remove_key(ptr);
    }
  }
